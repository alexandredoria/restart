...
class Ocorrencia extends DB {
	...
	public function cadastrarOcorrencia($descricao, $num_patrimonio, $lab, $num_posicionamento, $solicitante) {
		...
		if ($check = $this->db->query("SELECT num_patrimonio FROM patrimonio WHERE num_patrimonio = '$num_patrimonio'")) {
			if ($check->num_rows){
				$edit = $this->db->prepare("UPDATE patrimonio SET num_posicionamento = ?, id_laboratorio = ?, data_atualizacao = ? WHERE num_patrimonio = ?");
				...
				if ($edit->execute()) { 
					...
					$insert->bind_param('sissss', $descricao, $estado_servico, $data_cadastro, $data_cadastro, $num_patrimonio, $solicitante);
	...
	public function alterarOcorrencia($id, $descricao, $num_patrimonio, $lab, $num_posicionamento, $solicitante) {
		...
		if ($check = $this->db->query("SELECT num_patrimonio FROM patrimonio WHERE num_patrimonio = '$num_patrimonio'")) {
			if ($check->num_rows){
				...
				$edit->bind_param('iiss', $num_posicionamento, $lab, $data_atualizacao, $num_patrimonio);
				if ($edit->execute()) { 
					$insert = $this->db->prepare("UPDATE ocorrencia SET descricao = ?, data_atualizacao = ? WHERE id = ?");
					$insert->bind_param('ssi', $descricao, $data_atualizacao, $solicitante);
					if ($insert->execute()) { return true; }
					else { return ($this->db->error); }
				 }else { return ($this->db->error); }	
	...
	public function assumirOcorrencia($id, $matricula) {
		
		if ($check = $this->db->query("SELECT estado_servico FROM ocorrencia WHERE (estado_servico != 1) AND (id = ".$id.")")) {
			if ($check->num_rows) return "A ocorrência escolhida já não está mais aberta.";
			else {
				$estado_servico = 2;
				date_default_timezone_set("America/Bahia");	
				$data_atendimento = date("Y-m-d H:i:s", time());  

				$edit = $this->db->prepare("UPDATE ocorrencia SET estado_servico = ?, bolsista_alocado = ?, data_atendimento = ? WHERE id = ?");
				$edit->bind_param('issi', $estado_servico, $matricula, $data_atendimento, $id);
	...
	public function rejeitarOcorrencia($id) {
		...
		if ($check = $this->db->query("SELECT estado_servico FROM ocorrencia WHERE ((estado_servico != 2) AND (estado_servico != 4)) AND (id = ".$id.")")) {
			if ($check->num_rows) return "A ocorrência escolhida foi excluída pelo solicitante.";
				...
				$estado_servico = 1;
				$edit = $this->db->prepare("UPDATE ocorrencia SET estado_servico = ?, bolsista_alocado = NULL, data_atendimento = NULL WHERE id = ?");
	...
	public function notificarOcorrencias($matricula, $tipo, $pagina = 1) {
		...
		if ($tipo == 1){
				...
				$result	= $this->db->query($sql." WHERE (o.`estado_servico` = 1) OR ((o.`estado_servico` = 4) AND (bolsista_alocado = $matricula))ORDER BY o.`data_cadastro` DESC");
		} else if ($tipo == 3){
	...
	public function checkOcorrencia($nome) {
		...
		if ($check = $this->db->query("SELECT nome FROM Ocorrencia WHERE nome = '$prod'")) {
