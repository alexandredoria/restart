...
class Cliente extends DB {
	...
	public function cadastrarCliente($nome, $sobrenome,  $doc, $insc_estado, $cep, $endereco, $bairro, $uf, $cidade, $email, $tel, $notes, $user) {
		...
		$insert = $this->db->prepare("INSERT INTO clientes (nome_razao, num_documento, insc_estadual, cep, logradouro, bairro, estado, cidade, email, telefone, anotacoes, usuario_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
		$insert->bind_param('sssssssssssi',$nome, $sobrenome,  $doc, $insc_estado, $cep, $endereco, $bairro, $uf, $cidade, $email, $tel, $notes, $user);
